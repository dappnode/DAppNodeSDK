...
export function getUpstreamVersionTag(versionsToUpdate: ComposeVersionsToUpdate): string | string[] {
    ...
    if (entries.length === 1) {
        ...
        return entries.map(([repoSlug, { newVersion }]) => `${repoSlug}@${newVersion}`);
    }
}