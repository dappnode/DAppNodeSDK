...
async function buildAndUpload({
  ...
  await shell(
    `cp docker-compose*.yml ${buildDir}docker-compose-${shortName}.yml`,
    { silent }
  );
  ...
  if (imageId && tarHash && tarHash === cacheTarHash) {
    log(
      `Skipping image save and compression stage, tarball ${imagePath} has been verified`
    );
  } else {
  ...
  const imageUpload = await ipfs
    .addFromFs(imagePath, {
      pin: true,
      ...(showProgress && !silent
        ? { progress: logProgress(imagePath, log) }
        : {})
    })
    .then(res => res[0]);
  // Edit manifest
  ...
  const manifestUpload = await ipfs
    .addFromFs(manifestPath, { pin: true })
    .then(res => res[0]);
  // Write manifest IPFS upload results = {path, hash, size}
  fs.writeFileSync(
    `${buildDir}/upload.json`,
    JSON.stringify(manifestUpload, null, 2)
  );
