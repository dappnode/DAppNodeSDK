...
`);
  ...
  const answers = useDefaults ? defaultAnswers : await inquirer.prompt([
    {
      type: 'input',
      name: 'name',
    ...
    },
    {
      ...
      validate: (val) => (!fs.existsSync(val) && val != '') ? 'the avatar must be an png or jpg in the local path. You can leave this field empty' : true,
    },
    {
      type: 'list',
      name: 'type',
  ...
  fs.writeFileSync(`${path}/build/Dockerfile`, `FROM alpine
