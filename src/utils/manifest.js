 ...
 */
function getManifestPath({dir = './', manifestFileName = MANIFEST_NAME} = {}) {
...
function readManifest({dir, manifestFileName} = {}) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      throw Error(`No manifest found at ${path}. Make sure you are in a directory with an initialized DNP.`);
    } else {
      throw e;
    }
  }
