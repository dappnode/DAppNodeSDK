...
import {
  ...
  gitignoreData,
  gitignoreName
} from "../../src/commands/init/params.js";
import {
...
describe("Variant initialization", function () {
  ...
  it("checks for specific variant manifest", () => {
    const variantManifestPath = path.join(
      defaultVariantsDirName,
      defaultVariantsEnvValues[0],
      defaultManifestFileName
    );
    const rootManifestContent = JSON.parse(
      readTestFile(defaultManifestFileName)
    );
    const variantManifestContent = JSON.parse(
      readTestFile(variantManifestPath)
    );
